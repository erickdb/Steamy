...
local Window = Fluent:CreateWindow({
    ...
    TabWidth = tabWidth,
    Size = windowSize,
    Acrylic = not isMobile, -- Disable blur on mobile to prevent lag
    Theme = "Dark",
...
local function createMobileToggle()
    if CoreGui:FindFirstChild("SubterraToggleGui") then
        pcall(function() CoreGui.SubterraToggleGui:Destroy() end)
    end
    
    local MobileToggleGui = Instance.new("ScreenGui")
    MobileToggleGui.Name = "SubterraToggleGui"
    ...
    ToggleBtn.Image = "rbxassetid://17255152203" -- Pickaxe icon
...
-- Inner UI elements centering fix
local function findWindowFrame()
    local targets = {CoreGui, localPlayer and localPlayer:FindFirstChild("PlayerGui")}
    for _, parent in ipairs(targets) do
        if parent then
            for _, gui in ipairs(parent:GetChildren()) do
                if gui:IsA("ScreenGui") then
                    for _, desc in ipairs(gui:GetDescendants()) do
                        if desc:IsA("TextLabel") and (desc.Text:match("Subterra") or desc.Text:match("Steamy")) then
                            local current = desc
                            while current and current ~= gui do
                                if current:IsA("GuiObject") and current.Size.X.Scale == 0 and current.Size.X.Offset > 200 then
                                    return current, gui
                                end
...
task.spawn(function()
    local centeredCount = 0
    while centeredCount < 50 do
        task.wait(0.1)
        local mainFrame, gui = findWindowFrame()
        if mainFrame and mainFrame.Size.X.Offset > 0 then
            if gui then
...
-- Inner UI scaling and scrolling fix loop
task.spawn(function()
    while true do
        task.wait(1.5)
        local mainFrame, fluentGui = findWindowFrame()
        if fluentGui then
            for _, sf in ipairs(fluentGui:GetDescendants()) do
                if sf:IsA("ScrollingFrame") then
                    sf.Active = true
                end
            end
            
            for _, child in ipairs(fluentGui:GetDescendants()) do
                if child:IsA("GuiObject") then
                    if not child:IsA("ScrollingFrame") and child.Active then
                        child.Active = false
                    end
                    
                    if isMobile and not child:GetAttribute("OptimizedMobile") then
                        child:SetAttribute("OptimizedMobile", true)
                        
                        if child:IsA("TextLabel") or child:IsA("TextBox") or child:IsA("TextButton") then
                            if child.TextSize > 10 then
                                child.TextSize = math.max(10, child.TextSize - 2)
                            end
                        end
                        
                        if child:IsA("UIPadding") then
                            child.PaddingTop = UDim.new(0, math.max(1, child.PaddingTop.Offset - 3))
                            child.PaddingBottom = UDim.new(0, math.max(1, child.PaddingBottom.Offset - 3))
                            child.PaddingLeft = UDim.new(0, math.max(3, child.PaddingLeft.Offset - 5))
                            child.PaddingRight = UDim.new(0, math.max(3, child.PaddingRight.Offset - 5))
                        end
                        
                        if child:IsA("UIListLayout") then
                            child.Padding = UDim.new(0, math.max(1, child.Padding.Offset - 3))
                        end
                        ...
                        if child:IsA("TextButton") and child.Parent and child.Parent.Name == "TabHolder" then
                            if child.Size.Y.Offset > 24 then
                                child.Size = UDim2.new(child.Size.X.Scale, child.Size.X.Offset, 0, 24)
                            end
                        end
                    end
                end
...
Tabs.Main:AddSection("Movement Settings")
...
Tabs.Main:AddSection("Player Teleport")
...
local function pressSlot(slotNum)
    ...
    if kc then
        ...
        -- Press down with elevated privileges
        if setidentity then pcall(function() setidentity(8) end) end
        vim:SendKeyEvent(true, kc, false, game)
        if setidentity then pcall(function() setidentity(oldIdentity) end) end
        
        task.wait(0.05)
        
        -- Release with elevated privileges
        if setidentity then pcall(function() setidentity(8) end) end
...
local oreList = {
    "AgateBlock", "AmethystBlock", "AmethystOre", "AquamarineOre", "ArcticSnow",
    "ArgentiteOre", "AuroraliteOre", "AzuriteOre", "BigPresentGreen", "BigPresentRed",
    "BigPresentYellow", "BioluxiteOre", "BlackDiamondOre", "BlackIce", "BlueIce",
    "BlueIce2", "CandyCaneBlock", "CandyCaneDecor", "CelestiteOre", "ChalcedonyOre",
    "ChargedIce", "ChargedSchwarzFrost", "ChromiumOre", "Chronosium", "CitrineOre",
    "CoalOre", "CobaltOre", "ColdIce", "CopperOre", "CryoliumOre", "DiamondOre",
    "EiskronOre", "EmeraldOre", "Ferns", "FrozenVine", "GlacieliteOre", "GlowVine",
    "GoldOre", "GoldOreDarkstone", "HalocryteCrystal", "Ice", "IceBlock", "IcePot",
    "IcicleB", "IcicleU", "IronOre", "LeavesBlock", "LeavesSlab", "LongCrystal",
    "MoonstoneOre", "MycoriaOre", "NephriteOre", "NiveliumOre", "NocturniteOre",
    "OrichalcumOre", "PlatinumOre", "PlatinumOreDark", "Pot", "Roots", "RubyOre",
    "SaphireOre", "SapphireOre", "SaphrondOre", "SchwarzAquamarineOre", "SchwarzAuroraliteOre",
    "SchwarzChrono", "SchwarzFrost", "SchwarzfrostSlab", "SilverOre", "SmallPresentGreen",
    "SmallPresentRed", "SmallPresentYellow", "SnowBlock", "SoftIce", "SunstoneOre",
...
Tabs.AutoFarm:AddSection("Ores Auto Farm")
...
local OreDropdown
local FarmOreSearch = Tabs.AutoFarm:AddInput("FarmOreSearch", {
...
OreDropdown = Tabs.AutoFarm:AddDropdown("OreDropdown", {
...
Tabs.AutoFarm:AddSection("Mobs Auto Farm")
...
Tabs.Combat:AddToggle("KillAuraToggle", {
    Title = "⚔️ Kill Aura",
    Description = "Serang otomatis semua monster di sekitar tanpa perlu mengeker",
    Default = false,
    Callback = function(Value)
...
Tabs.Combat:AddSlider("KillAuraRangeSlider", {
    Title = "Kill Aura Range",
    Description = "Jangkauan serangan Kill Aura (dalam studs)",
    Min = 5,
    Max = 35,
    Default = 15,
    Rounding = 0,
...
Tabs.Combat:AddSlider("KillAuraHitRateSlider", {
    Title = "Kill Aura Hit Speed (Damage Multiplier)",
    Description = "Jumlah serangan instan per monster (Semakin tinggi semakin sakit/double damage)",
    Min = 1,
    Max = 10,
    Default = 3,
    Rounding = 0,
...
Tabs.Combat:AddToggle("AutoBlockToggle", {
    Title = "🛡️ Auto Block (Shield)",
    Description = "Otomatis menggunakan perisai (shield) untuk menangkis saat terkena serangan monster",
    Default = false,
    Callback = function(Value)
...
Tabs.Combat:AddSection("Auto Healing")

Tabs.Combat:AddToggle("AutoHealToggle", {
    Title = "❤️ Auto Heal",
...
Tabs.Combat:AddDropdown("HealPotionDropdown", {
...
Tabs.Combat:AddSlider("HealHealthSlider", {
...
-- Auto Sell Tab
Tabs.AutoSell:AddSection("Auto Sell Settings")
...
local SellDropdown
local SellSearch = Tabs.AutoSell:AddInput("SellSearch", {
...
SellDropdown = Tabs.AutoSell:AddDropdown("SellDropdown", {
...
local function updateYrsaESP()
    ...
    if YrsaStatusParagraph then
        if yrsaSpawnedModel then
            ...
            YrsaStatusParagraph:SetTitle("👑 Yrsa Terdeteksi (SPAWNED)!")
            YrsaStatusParagraph:SetDesc(string.format("Posisi: %d, %d, %d | Jarak: %d m", 
            ...
            YrsaStatusParagraph:SetTitle("Yrsa Boss Status")
            YrsaStatusParagraph:SetDesc("Yrsa: Belum Muncul / Belum Di-summon")
        end
    end
...
Tabs.ESP:AddSection("👑 Yrsa Boss ESP")

Tabs.ESP:AddToggle("YrsaESPToggle", {
    Title = "Yrsa ESP",
...
YrsaStatusParagraph = Tabs.ESP:AddParagraph({
    Title = "Yrsa Boss Status",
    Content = "Menunggu pemindaian..."
})
...
local OreESPFilter
local OreESPSearch = Tabs.ESP:AddInput("OreESPSearch", {
...
OreESPFilter = Tabs.ESP:AddDropdown("OreESPFilter", {
...
local KeybindOption = Tabs.Settings:AddKeybind("MinimizeKeybind", {
    Title = "UI Toggle Keybind",
    Description = "Tombol untuk menyembunyikan/menampilkan menu UI",
    Mode = "Toggle", -- Always Toggle
    Default = "RightShift",
    Callback = function(Value)
        -- Fluent will update internal minimize key
    end,
    ChangedCallback = function(NewKey)
        Window.MinimizeKey = NewKey
    end
})
Window.MinimizeKey = Enum.KeyCode.RightShift
...
-- Fix Fluent UI first tab header text bug
task.spawn(function()
    for i = 1, 15 do
        local screenGui = game:GetService("CoreGui"):FindFirstChild("ScreenGui")
        if screenGui then
            for _, desc in ipairs(screenGui:GetDescendants()) do
                if desc:IsA("TextLabel") and desc.Text == "Tab" then
                    desc.Text = "Welcome, " .. localPlayer.DisplayName
                end
            end
        end
        task.wait(0.3)
